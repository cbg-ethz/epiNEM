...
for (i in 1:runs) {
    ...
    for (j in 1:length(noiselvls)) {
        ...
        if ("b" %in% do) {
            ...
            nodes <-
            unique(colnames(sortedData)[-grep("\\.", colnames(sortedData))])
            ...
            if (length(grep("S", rownames(gtn2))) > 0) {
                gtn2 <-
                gtn2[-grep("S", rownames(gtn2)), -grep("S", colnames(gtn2))]
            }
            ...
            if (nrow(res) < nrow(gtn2)) {
                res2 <-
                rbind(cbind(res, matrix(0, nrow(res), nrow(gtn2) - nrow(res))),
        ...
        if (any(c("n", "p", "a") %in% do)) {
            ...
            for (k in 1:length(unique(colnames(reddata)))) {
                reddata2[, k] <-
                as.vector(reddata[, which(colnames(reddata) %in%
...
for (i in doubles[set]) {
    ...
    egenes <- which(apply(dataBin[,
              which(colnames(dataBin) %in% c(i, doubles.singles))], 1, max)
              == 1)
    for (j in singles) {
        print(j)
        if (j %in% doubles.singles) { next() }
        ...
        dataTmp <-
            dataBin[, grep(paste(paste("^", c(i, j, doubles.singles), "$",
            sep = ""), collapse = "|"), colnames(dataBin))]
