...
plot(res)
```
```{r, fig.width = 5, fig.height = 5, fig.cap = fig.cap3}
...
plot(samscreen, xrot = 45, cexRow = 0.5, borderwidth = 0)
```
...
```
```{r, fig.width = 8, fig.height = 5, fig.cap = fig.cap11}
data(sameith_string)
...
The next analysis is similar to the STRINGdb interaction scores. However,
instead of these we use Graph-based GO term similarity scores. Additionally,
we do not compare a pair of genes, but we compare the GO similarity between
the pair of the double knock-out and the third modulator.
...
In the final analysis, we investigate the GO enrichment of the modulators'
E-gene targets. These E-gene are identified by epiNEM to be directly
regulated by the modulator and not the parent genes.
...
We do the same analysis with the Sameith et al., results.
...
pathway enrichment. Fig. 16, 17 show heatmaps of false discovery rates.
...
GO terms, Fig. 18 shows enriched pathways (rows) for effect reporters of the
