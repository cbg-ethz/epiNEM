...
In the final analysis, we investigate the GO enrichment of the modulators E-gene
targets. These E-gene are identified by epiNEM to be directly regulated by the
modulator and not the parent genes.
...
for (i in 1:length(wagscreen$targets)) {
    ...
    for (j in 1:length(wagscreen$targets[[i]])) {
        egenego[[i]][[j]] <- string_db$get_enrichment(string_db$mp(wagscreen$targets[[i]][[j]]),
                                 category = "KEGG", methodMT = "fdr", iea = TRUE)
...
for (i in 1:length(wagscreen$targets)) {
    for (j in 1:length(wagscreen$targets[[i]])) {
        if (dim(egenego[[i]][[j]])[1] > 0) {
            gomat[which(rownames(gomat) %in% egenego[[i]][[j]]$term_description), which(dimnames(gomat)[[2]] %in% names(wagscreen$targets[[i]])[j]), i] <- egenego[[i]][[j]]$pvalue_fdr[order(egenego[[i]][[j]]$term_description)]
        }
    }
}
...
gomat <- gomat[rev(order(apply(gomat, 1, function(x) return(sum(is.na(x) == FALSE))))), ]

gomat[which(is.na(gomat) == TRUE)] <- -max(gomat, na.rm = TRUE)

HeatmapOP(gomat, xrot = 45, Colv = F, Rowv = F)
```
...
for (i in 1:length(samscreen$targets)) {
    ...
    for (j in 1:length(samscreen$targets[[i]])) {
        egenego2[[i]][[j]] <- string_db$get_enrichment(string_db$mp(samscreen$targets[[i]][[j]]),
                                 category = "KEGG", methodMT = "fdr", iea = TRUE)
...
for (i in 1:length(samscreen$targets)) {
    for (j in 1:length(samscreen$targets[[i]])) {
        if (dim(egenego[[i]][[j]])[1] > 0) {
            gomat[which(rownames(gomat) %in% egenego[[i]][[j]]$term_description), which(dimnames(gomat)[[2]] %in% names(samscreen$targets[[i]])[j]), i] <- egenego[[i]][[j]]$pvalue_fdr[order(egenego[[i]][[j]]$term_description)]
        }
    }
}
