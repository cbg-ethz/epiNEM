...
wagscreen$doubles <- wagscreen$doubles[-grep("fus3|ptp2.ptc2", wagscreen$doubles)]

wagscreen$dataWag <- wagscreen$dataWag[, -grep("fus3|ptp2.ptc2", colnames(wagscreen$dataWag))]

wagscreen$ll <- wagscreen$ll[, -grep("fus3|ptp2.ptc2", colnames(wagscreen$ll))]

wagscreen$logic <- wagscreen$logic[, -grep("fus3|ptp2.ptc2", colnames(wagscreen$logic))]
...
golist[[i]] <- string_db$get_enrichment(string_db$mp(unique(c(unlist(strsplit(
colnames(wagscreen$ll)[i], "\\.")),
...
goterms <- c(goterms,
golist[[i]]$term_description[which(golist[[i]]$pvalue_fdr < 1)]) 
...
dev.print("Figure11a.pdf", width = 8, height = 5, device = pdf)
```
```{r, eval=FALSE}
## Sameith:
...
golist2[[i]] <- string_db$get_enrichment(string_db$mp(unique(c(unlist(strsplit(
...
goterms <-
c(goterms, golist2[[i]]$term_description[which(golist2[[i]]$pvalue_fdr < 0.1)])
...
golist2[[i]][which(golist2[[i]]$term_description %in% rownames(gomat)), 4]
...
pvals[i] <- wilcox.test(base, sample(base, 100), alternative = "greater")$p.value
...
dev.print("Figure10b.pdf", width = 12, height = 10, device = pdf)
...
for (i in 1:ncol(wagscreen$ll)) {
    ...
    for (j in which(!is.infinite(wagscreen$ll[, i]) == TRUE &
                    wagscreen$ll[, i] != 0)) {
        tmp <- geneSim(pair[1], toupper(rownames(wagscreen$ll)[j]),
                       semData = ystGO, drop = NULL)
        if (!is.na(tmp[1])) {
            GOepi <- c(GOepi, tmp$geneSim)
        }
        tmp <- geneSim(pair[2], toupper(rownames(wagscreen$ll)[j]),
                       semData = ystGO, drop = NULL)
        if (!is.na(tmp[1])) {
            GOepi <- c(GOepi, tmp$geneSim)
        }
...
allgenes <- toupper(unique(c(rownames(wagscreen$ll),
                             unlist(strsplit(colnames(wagscreen$ll), "\\.")))))

for (i in 1:(length(allgenes) - 1)) {
    g1 <- allgenes[i]
    for (j in (i+1):length(allgenes)) {
        g2 <- allgenes[j]
        tmp <- geneSim(g1, g2, semData = ystGO, drop = NULL)
        if (!is.na(tmp[1])) {
            if (i == j) { print(i); print(j) }
...
for (i in 1:ncol(samscreen$ll)) {
    ...
    for (j in which(!is.infinite(samscreen$ll[, i]) == TRUE &
                    samscreen$ll[, i] != 0)) {
        tmp <- geneSim(pair[1], toupper(rownames(samscreen$ll)[j]),
                       semData = ystGO, drop = NULL)
        if (!is.na(tmp[1])) {
            GOepi2 <- c(GOepi2, tmp$geneSim)
        }
        tmp <- geneSim(pair[2], toupper(rownames(samscreen$ll)[j]),
                       semData = ystGO, drop = NULL)
        if (!is.na(tmp[1])) {
            GOepi2 <- c(GOepi2, tmp$geneSim)
        }
...
allgenes <- toupper(unique(c(rownames(samscreen$ll),
                             unlist(strsplit(colnames(samscreen$ll), "\\.")))))
