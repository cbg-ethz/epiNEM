...
We generate a matrix visualizing the KEGG pathway enrichment for all pairs for the van
...
for (i in 1:length(golist)) {
    ...
    goterms <- c(goterms,
                 golist[[i]]$term_description
                 [which(golist[[i]]$pvalue_fdr < 1)])
...
for (i in 1:ncol(sameith$ll)) {
    ...
    gomat[which(rownames(gomat) %in% golist2[[i]]$term_description), i] <-
        golist2[[i]]
    [which(golist2[[i]]$term_description %in% rownames(gomat)), 4]
