...
while NEM, ARACNE and the P algorithm are much faster. However, the last
...
strucutres in the case of B-NEM.
...
```
We plot the densities of the distributions of the random triplet
scores (blue) and the scores of the triplets by epiNEM (pink). Additional
we calculate, if the difference between the two score distributions is
...
In addition to STRINGdb interaction scores, we have a look at enrichmed
GO terms. For this analysis we again use the STRINGdb package.

We generate a matrix visualizing the GO terms for all pairs for the van
Wageningen and the Sameith data sets.

```{r, eval=FALSE}
## van Wageningen:

string_db$set_background(
string_db$mp(unique(c(unlist(strsplit(colnames(wageningen$ll), "\\.")),
rownames(wageningen$ll)))))

golist <- list()

for (i in 1:ncol(wageningen$ll)) {
golist[[i]] <- string_db$get_enrichment(string_db$mp(c(unlist(strsplit(
colnames(wageningen$ll)[i], "\\.")),
rownames(wageningen$ll)[which(!(wageningen$logic[, i] %in% c("NOINFO", "NOEPI")))])),
category = "Process", methodMT = "fdr", iea = TRUE)   
}
```
...
goterms <- c(goterms,
golist[[i]]$term_id[which(golist[[i]]$pvalue_fdr < 1)]) 

}

gomat <- matrix(0, length(unique(goterms)), ncol(wageningen$ll))

rownames(gomat) <- sort(unique(goterms))
colnames(gomat) <- colnames(wageningen$ll)

## there is a strong cutoff for fdr >= 0.05 !?!
...
gotmp <- golist[[i]]
gotmp <- gotmp[order(gotmp$term_id), ]

gomat[which(rownames(gomat) %in% golist[[i]]$term_id), i] <-
golist[[i]][which(golist[[i]]$term_id %in% rownames(gomat)), 4]

}

rownames(gomat) <- NULL

gomat[which(gomat == 0)] <- 1

gomat <- gomat[, -grep("fus3", colnames(gomat))]

HeatmapOP(gomat, breaks = seq(0,10^-1*0.6, length = 100),
bordercol = "transparent",
main = "False discovery rates of GO terms over all double knock-out pairs",
sub = "fdr cutoff at 0.05",
xrot = 45, col = "Blues")
```
```{r, eval=FALSE}
## Sameith:
...
golist2[[i]] <- string_db$get_enrichment(string_db$mp(c(unlist(strsplit(
colnames(sameith$ll)[i], "\\.")),
rownames(sameith$ll)[which(!(sameith$logic[, i] %in% c("NOINFO", "NOEPI")))])),
category = "Process", methodMT = "fdr", iea = TRUE)
}
```
```{r, fig.width = 8, fig.height = 7}
...
goterms <-
c(goterms, golist2[[i]]$term_id[which(golist2[[i]]$pvalue_fdr < 0.1)])

}

gomat <- matrix(0, length(unique(goterms)), ncol(sameith$ll))
...
gomat[which(rownames(gomat) %in% golist2[[i]]$term_id), i] <-
golist2[[i]][which(golist2[[i]]$term_id %in% rownames(gomat)), 4]
...
instead of these we use Graph-based GO term similarity scores.
...
keytype = keytypes(org.Sc.sgd.db)[11])
```
```{r, eval=FALSE}
GOepi <- numeric()
...
pair <- toupper(unlist(strsplit(colnames(wageningen$ll)[i], "\\.")))

for (j in which(!is.infinite(wageningen$ll[, i]) == TRUE &
wageningen$ll[, i] != 0)) {

tmp <- geneSim(pair[1], toupper(rownames(wageningen$ll)[j]),
semData = ystGO)

if (!is.na(tmp[1])) {

GOepi <- c(GOepi, tmp$geneSim)
...
for (i in toupper(unique(c(rownames(wageningen$ll),
toupper(unlist(strsplit(colnames(wageningen$ll), "\\."))))))) {

for (j in toupper(unique(c(rownames(wageningen$ll),
toupper(unlist(strsplit(colnames(wageningen$ll), "\\."))))))) {

if (i %in% j) { next() }

tmp <- geneSim(i, j, semData = ystGO)

if (!is.na(tmp[1])) {

GOall <- c(GOall, tmp$geneSim)
...
pair <- toupper(unlist(strsplit(colnames(sameith$ll)[i], "\\.")))

for (j in which(!is.infinite(sameith$ll[, i]) == TRUE &
sameith$ll[, i] != 0)) {

tmp <- geneSim(pair[1], toupper(rownames(sameith$ll)[j]),
semData = ystGO)

if (!is.na(tmp[1])) {

GOepi2 <- c(GOepi2, tmp$geneSim)
...
for (i in toupper(unique(c(rownames(sameith$ll),
toupper(unlist(strsplit(colnames(sameith$ll), "\\."))))))) {

for (j in toupper(unique(c(rownames(sameith$ll),
toupper(unlist(strsplit(colnames(sameith$ll), "\\."))))))) {
